...
class LudiiTranspiler:
    ...
    def _transpile_foreach_piece(self, play_text: str) -> str:
        ...
        # Only look at the equipment section for movement keywords — NOT end conditions
        equip_text = ""
        if hasattr(self, '_tree'):
            equip = _find_child(self._tree, "equipment")
            if equip:
                equip_text = _get_text(equip)

        moves = []
        piece_name = self.pieces[0][0] if self.pieces else "token"

        has_step = "move Step" in equip_text
        has_hop = "move Hop" in equip_text or "Hop" in equip_text
        has_slide = "move Slide" in equip_text or "Slide" in equip_text
        has_forward = "Forward" in equip_text or "FR" in equip_text or "FL" in equip_text
